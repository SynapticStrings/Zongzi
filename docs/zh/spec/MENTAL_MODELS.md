...
| **Host** | 库外编排者（任意） | 持 Note 表；edit 后组 Context；`rebase_all` → window → check/render；上浮 conflicts |
| **Engine** | `check_whole` 必选；partial/render 可选 | check（语义/参数）与 render（重产物）分层 artifact |
...
Slice     = 「这一锅 [start,end) + seq_ids」（批处理，瞬态）
```
...
- **`Windowing.Strategy`**：单一回调 `window(Context) → {:ok, [Slice]} | error`（模块内可私有组合；**不是** atom plug 管道）。  
- **`Slice`**：`start_tick` / `end_tick` 左闭右开 + `seq_ids`。  
...
```text
edit → Timeline → rebase_all → Strategy.window(survived ivs)
     → Engine.check_* →（决议）→ render_*
```
...
- `check_whole/1`（必选）/ `check_partial/1`（可选）— 轻量语义与参数检查。  
- `render_whole/1` / `render_partial/1`（可选）— 重渲染；artifact ≠ check 产出。  
- Intervention = 可改的上游生成结果（含可编辑 G2P）；Gender/Energy 等走 params 约束。

### `seq_to_window`（Anchor.Context）

rebase 时可选、**上一轮**启发式；本轮切片只来自 post-rebase 的 `Strategy.window/1`。

### 现状

`Windowing.Strategy` / `RestSplit3Beats` / `WholeTrack` 已落地。`Score.Slicer` 仍是 note-only 旧工具，不作默认 phrase 定义。
...
- 引擎 pad 表、phrase 缓存键、邻片失效策略 — Host/引擎；zongzi 只提供 Strategy 与默认切开规则，不把产品缓存写进 Timeline
