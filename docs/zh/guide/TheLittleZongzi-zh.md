...
zongzi 是一个应用于 SVS 应用的内核/接口，其针对调教过程中音符变化后（拖拽等）调教参数静默失效的情况而被设计，旨在将失效行为暴露给用户让其定夺。

### 除了这个仓库还有什么？

可以看 <https://github.com/GES233/zongzi_feasibiliity> ，以及后续梳理完成后重构的代码以及帮助文档。
...
### 如果我改一个 note 的歌词，Timeline 本身会变吗？
...
### 为什么叫 Intervention （下文用 Interv 简写代替）？什么才算做 Interv ？
...
尝试处理这种隐含的错误或冲突，也是开发 zongzi 的主要原因。

### Interv 绑在哪个 note 上？（anchor 三元组是什么）
...
`:payload` 里存着的，就是源于用户的 Interv 数据本身。
...
### snapshot 是什么时候拍的？为什么要拍？

这是对原始输出数据的一个快照或剪影。snapshot 对得上，interv 就有效。
...
简单说就是 interv 的范围，是否可能「污染」到前面或后面的音符。

### strategy 字段是干嘛的？

那就是针对这个 interv 执行锚定所要走的策略和参数了。值是一个 `{module(), options}` 元组，例如 `{NoteTriplet, %NoteTriplet.Options{match_threshold: 1}}`。`nil` 时 dispatch 用默认策略（`NoteTriplet` + 默认参数）。

## Phase 3 —— 谱表操作之后的锚定、同步与变基
...
### "结构层冲突"是指什么？（preserve / rebase / relocate / split / conflict 各是什么场景）
...
- `:relocate` - focus 已死（被删/被合并），若 `orphan_direction` 为 `:prev`/`:next` 则重新挂到最近的活跃邻居上；默认 `:never` 时直接走 conflict（见下文流程图）
...
flowchart TD
    ...
    m -->|delete_tombstone| od{"orphan_direction<br/>（默认 :never）"}
...
### split note 时，挂在原 note 上的 Intervention 怎么处理？

按照我本来的想法是附近的都无效掉，但对于部分 interv 不用全部 discard 掉。所以可以交给 declara 的可选 callback `on_rebase/4` 来实现。

### delete note 时，Intervention 会怎样？
...
默认情况下（`orphan_direction: :never`），delete tombstone 直接报 conflict（`:relocate_forbidden`），不尝试 relocate。
只有当 Caller 显式将 `orphan_direction` 设为 `:prev` 或 `:next` 时，才会向指定方向（及反向作为 fallback）扫描最近的活跃邻居，尝试 relocate。

以下是非默认配置（`orphan_direction: :next`）下的 relocate 示例：
...
### rebase_all 里面走了哪两步？和渲染时的 resolve 怎么分界？
...
### Windowing 把 Timeline 切成什么给 Engine？为什么要切？

将整个 Timeline 以及音符序列切成片段，以便于外部应用实现基于乐句的缓存与增量生成等特性，同时出于简化 API 以及兼容，哪怕不需要外部缓存，也会将整轨压缩成一个完整片段。
...
### Declaration behaviour 要求实现哪几个回调？（scope / snapshot / resolve）
...
### resolve 什么时候返回 {:ok, applied}，什么时候返回 {:conflict, :snapshot_stale}？

### snapshot 的归一化做了什么？为什么需要归一化？
