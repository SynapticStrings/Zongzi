defmodule Zongzi.Timeline.QueryTest do
  ...
  describe "status/2" do
    test "active" do
      ...
      [a, b, c] = notes
      assert Timeline.status(tl, a.seq_id) == :active
      assert Timeline.status(tl, b.seq_id) == :active
      assert Timeline.status(tl, c.seq_id) == :active
    end

    test "missing for unknown seq_id" do
      {tl, _} = build_tl([note()])
      assert Timeline.status(tl, 99999) == :missing
    end
    ...
    test "merge_tombstone after merge" do
      ...
      assert Timeline.status(tl, a.seq_id) == :active
      assert Timeline.status(tl, b.seq_id) == :merge_tombstone
    end
    ...
    test "delete_tombstone after delete" do
      ...
      assert Timeline.status(tl, a.seq_id) == :delete_tombstone
      assert Timeline.status(tl, b.seq_id) == :active
    end
    ...
    test "active? alias" do
      ...
      [a] = notes
      assert Timeline.active?(tl, a.seq_id)
      refute Timeline.active?(tl, a.seq_id + 999)
    end
  end
  ...
  describe "scan/4" do
    test "prev direction active_only" do
      {tl, notes} = build_tl([note(), note(), note()])
      [a, b, c] = notes
      assert Timeline.scan(tl, c.seq_id, :prev) == [b.seq_id, a.seq_id]
    end

    test "next direction active_only" do
      {tl, notes} = build_tl([note(), note(), note()])
      [a, b, c] = notes
      assert Timeline.scan(tl, a.seq_id, :next) == [b.seq_id, c.seq_id]
    end

    test "limit" do
      {tl, notes} = build_tl([note(), note(), note(), note()])
      [_a, _b, _c, d] = notes
      result = Timeline.scan(tl, d.seq_id, :prev, limit: 2)
    ...
    test "max_hops" do
      {tl, notes} = build_tl([note(), note(), note()])
      [_a, _b, c] = notes
      result = Timeline.scan(tl, c.seq_id, :prev, max_hops: 1)
    ...
    test "include_self" do
      {tl, notes} = build_tl([note(), note()])
      [a, _b] = notes
      result = Timeline.scan(tl, a.seq_id, :next, include_self: true, active_only: false)
    ...
    test "active_only false includes tombstones" do
      ...
      result = Timeline.scan(tl, a.seq_id, :next, active_only: false)
    ...
    test "active_only true skips tombstones" do
      ...
      [a, b, c] = notes
      {:ok, tl} = Timeline.delete_note(tl, b.seq_id)
      result = Timeline.scan(tl, a.seq_id, :next, active_only: true)
    ...
    test "missing start returns empty" do
      {tl, _} = build_tl([note()])
      assert Timeline.scan(tl, 99999, :next) == []
    end
  end
  ...
  describe "neighborhood/3" do
    test "default count:1 active_only:false mirrors adjacent raw neighbors" do
      {tl, notes} = build_tl([note(), note(), note()])
      [a, b, c] = notes
      nb = Timeline.neighborhood(tl, b.seq_id)
    ...
    test "count:2 collects two per side" do
      {tl, notes} = build_tl([note(), note(), note(), note()])
      [_a, b, _c, _d] = notes
      nb = Timeline.neighborhood(tl, b.seq_id, count: 2)
      assert length(nb.left) == 1
    ...
    test "active_only:true skips tombstones" do
      ...
      [a, b, c] = notes
      {:ok, tl} = Timeline.delete_note(tl, b.seq_id)
      nb = Timeline.neighborhood(tl, a.seq_id, active_only: true, count: 2)
    ...
    test "focus at head has empty left" do
      {tl, notes} = build_tl([note(), note()])
      [a, _b] = notes
      nb = Timeline.neighborhood(tl, a.seq_id)
      assert nb.left == []
    end
    ...
    test "missing focus" do
      {tl, _} = build_tl([note()])
      nb = Timeline.neighborhood(tl, 99999)
  ...
  describe "scrub_triplet/2" do
    test "active focus gets clean triplet" do
      {tl, notes} = build_tl([note(), note(), note()])
      [a, b, c] = notes
      assert Timeline.scrub_triplet(tl, b.seq_id) == {:ok, {a.seq_id, b.seq_id, c.seq_id}}
    end
    ...
    test "scrubs tombstones from neighbors" do
      ...
      [a, b, c] = notes
      {:ok, tl} = Timeline.delete_note(tl, b.seq_id)
      assert Timeline.scrub_triplet(tl, c.seq_id) == {:ok, {a.seq_id, c.seq_id, nil}}
    end

    test "head focus gets nil prev" do
      {tl, notes} = build_tl([note(), note()])
      [a, b] = notes
      assert Timeline.scrub_triplet(tl, a.seq_id) == {:ok, {nil, a.seq_id, b.seq_id}}
    end

    test "tail focus gets nil next" do
      {tl, notes} = build_tl([note(), note()])
      [a, b] = notes
      assert Timeline.scrub_triplet(tl, b.seq_id) == {:ok, {a.seq_id, b.seq_id, nil}}
    end
    ...
    test "tombstone focus returns error" do
      ...
      [a, _b] = notes
      {:ok, tl} = Timeline.delete_note(tl, a.seq_id)
      assert Timeline.scrub_triplet(tl, a.seq_id) == {:error, :not_active}
    end
  end
  ...
  describe "hops/3" do
    test "adjacent notes have hop 1" do
      {tl, notes} = build_tl([note(), note()])
      [a, b] = notes
      assert Timeline.hops(tl, a.seq_id, b.seq_id) == {:ok, 1}
    end

    test "same note hop 0" do
      {tl, notes} = build_tl([note()])
      [a] = notes
      assert Timeline.hops(tl, a.seq_id, a.seq_id) == {:ok, 0}
    end
    ...
    test "tombstones counted in hops" do
      ...
      [a, b, c] = notes
      {:ok, tl} = Timeline.delete_note(tl, b.seq_id)
      assert Timeline.hops(tl, a.seq_id, c.seq_id) == {:ok, 2}
    end

    test "missing returns error" do
      {tl, notes} = build_tl([note()])
      [a] = notes
      assert Timeline.hops(tl, a.seq_id, 99999) == {:error, :not_found}
    end
  end
