...
| Engine **契约**（`render/1`） | Windowing 生产 `seq_to_window`（Timeline 分窗，未落地） |
| Slicer（纯 tick 切窗工具） | cross-channel invalidation 策略 |

**Host** 不是 zongzi 模块，而是库外编排者（典型：Equinox）。  
它持有 Note 表，在 edit 后组装 `Anchor.Context`，调用 `Anchor.rebase_all/4`，再组 `Engine.Request` 调引擎。
...
sequenceDiagram
    actor User
    participant Host as Host (Equinox 等)
    participant Zongzi
    ...
    loop 对抗轮
        ...
        Note over Host: scope 并集 / 切窗（Windowing，库外或未落地层）
        Host->>Engine: render(Request + rebased interventions)
        Note over Engine: projection → Declaration.resolve → apply/conflict
        Engine-->>Host: Artifactₙ ± semantic conflicts
        Host-->>User: Artifactₙ + 语义 conflicts（若有）
    end
