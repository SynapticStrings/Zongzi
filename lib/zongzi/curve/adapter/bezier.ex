defmodule Zongzi.Curve.Adapter.Bezier do
  ...
  defimpl Inner, for: __MODULE__ do
    ...
    def span(%{points: pts}), do: List.last(pts).tick

    # ---- rasterize ----

    def rasterize(%{points: []}, tick_seq) do
      for _ <- tick_seq, into: <<>>, do: <<0.0::float-32-native>>
    end

    def rasterize(%{points: [single]}, tick_seq) do
      v = single.value
      for _ <- tick_seq, into: <<>>, do: <<v::float-32-native>>
    end

    def rasterize(%{points: points}, tick_seq) do
