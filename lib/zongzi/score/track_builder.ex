defmodule Zongzi.Score.TrackBuilder do
  @moduledoc """
  Caller 侧「持轨」组件的文档锚点。

  本模块只提供文档，不提供实现。Zongzi 只管纯数据与校验，
  不替 Caller 选型其状态形态（GenServer / LiveView assigns / 纯数据管道），
  因此「持轨」——即 Caller 自持的一组随编辑共同演进的状态——无法也不应由库实现。
  本模块把 Caller 必须自己维护的事项写成契约；若未来 Caller 形态收敛，
  可在此长出共享 helper。
