defmodule Zongzi.Timeline do
  ...
  defp validate_head_tail(%__MODULE__{head: nil, tail: nil, nodes: nodes}) do
    if nodes == %{}, do: :ok, else: {:error, {:head_tail_nil_with_nodes, nodes}}
  end

  defp validate_head_tail(%__MODULE__{head: nil, tail: tail}) do
    {:error, {:head_tail_mismatch, nil, tail}}
  end

  defp validate_head_tail(%__MODULE__{head: head, tail: nil}) do
    {:error, {:head_tail_mismatch, head, nil}}
  end

  defp validate_head_tail(%__MODULE__{head: head, tail: tail, nodes: nodes}) do
    cond do
  ...
  defp validate_node_consistency(%__MODULE__{nodes: nodes}) do
  ...
  defp validate_seq_map_tombstones(%__MODULE__{
         nodes: nodes,
         seq_map: seq_map,
         tombstones: tombstones
       }) do
    with :ok <- validate_nodes_status(nodes, seq_map, tombstones),
         :ok <- validate_seq_map_refs(seq_map, nodes, tombstones) do
      :ok
    end
  end
  ...
  defp validate_seq_map_refs(seq_map, nodes, _tombstones) do
  ...
  defp validate_next_seq(%__MODULE__{next_seq: next_seq, nodes: nodes}) do
    if next_seq < 1 do
